...
MIN_LINES="${1:-8}"
DUPLO="$(command -v duplo 2>/dev/null || echo ./tools/duplo)"
if [[ ! -x "$DUPLO" ]]; then
...
LIST="$(mktemp)"
find services platform domain-shared -path '*/src/main/java/*' -name '*.java' -not -path '*/target/*' > "$LIST"
COUNT="$(wc -l < "$LIST")"
echo "Analyzing ${COUNT} Java files (min block = ${MIN_LINES} lines)..."
